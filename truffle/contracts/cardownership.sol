...
contract Ownership is ERC721, CardFactory {
    ...
    function ownerOf (uint256 _tokenId) external view returns (address) {
        return cardOwnership[_tokenId];
    }
    
    function _transfer (address _from, address _to, uint256 _tokenId) private {
        ownerCardCount[_to] = ownerCardCount[_to].add(1);
        ownerCardCount[msg.sender] = ownerCardCount[msg.sender].sub(1);
        cardOwnership[_tokenId] = _to;
        emit Transfer (_from, _to, _tokenId);
    }
    
    function transferFrom (address _from, address _to, uint256 _tokenId) public payable {
        require (cardOwnership[_tokenId] == msg.sender || cardApprovals[_tokenId] == msg.sender);
        _transfer(_from, _to, _tokenId);
    }
    // _tokenId is _indexId in the array
    function approve (address _approved, uint256 _tokenId) public payable {
        require(msg.sender == cardOwnership[_tokenId], "Only owner can set approval!!");
